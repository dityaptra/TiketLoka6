...
export default function PaymentPage() {
    ...
    const fetchBooking = useCallback(async () => {
        try {
            const res = await fetch(`http://127.0.0.1:8000/api/bookings/${params.code}`, {
    ...
    const handleCheckStatus = async () => {
        ...
        try {
            ...
            const res = await fetch(`http://127.0.0.1:8000/api/bookings/${booking?.booking_code}/pay`, {
                method: 'POST',
                headers: { 
    ...
    const handleCancelOrder = async () => {
        ...
        setProcessing(true);
        try {
            const res = await fetch(`http://127.0.0.1:8000/api/bookings/${booking?.booking_code}/cancel`, {
                method: 'POST',
                headers: { 'Authorization': `Bearer ${token}` }
            });
            if (res.ok) {
