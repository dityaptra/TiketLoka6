...
import {
  ...
  User,
  Copy,
  Check, // Import ikon Check (Centang)
} from "lucide-react";
...
export default function TicketDetailPage() {
  ...
  // STATE BARU: Untuk mengontrol status copy
  ...
  // FUNGSI BARU: Menangani logika salin kode
  const handleCopyCode = () => {
    if (booking?.booking_code) {
      navigator.clipboard.writeText(booking.booking_code);
      setIsCopied(true);

      // Kembalikan ke ikon copy setelah 2 detik (2000ms)
      setTimeout(() => {
        setIsCopied(false);
      }, 2000);
    }
    ...
    const fetchDetail = async () => {
      try {
        const res = await fetch(
          `http://127.0.0.1:8000/api/bookings/${bookingCode}`,
          {
            method: "GET",
            headers: {
  ...
  return (
    <main
      className={`min-h-screen ${bgPageColor} pb-20 font-sans text-gray-800`}
    >
      {/* --- NAVBAR --- */}
      <div className="bg-white px-4 py-3 shadow-sm sticky top-0 z-20 border-b border-gray-200">
        <div className="max-w-5xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Link
              href="/tickets"
              className="hover:bg-gray-100 p-2 rounded-full transition text-gray-600"
            >
          ...
          <div className="flex gap-2">
            <button
              ...
              className="flex items-center gap-2 text-gray-600 hover:bg-gray-100 px-3 py-1.5 rounded-lg text-sm font-medium transition"
            >
      ...
      <div className="max-w-5xl mx-auto px-4 mt-8 md:mt-12">
        ...
        <div className="relative w-full max-w-4xl mx-auto drop-shadow-2xl">
          ...
          <div className="flex flex-col md:flex-row bg-white rounded-3xl overflow-hidden">
            {/* === BAGIAN KIRI: DETAIL TIKET (70% Width) === */}
            <div className="flex-1 p-0 md:p-0 relative">
              ...
              <div className="bg-[#0B2F5E] p-6 text-white flex justify-between items-start relative overflow-hidden">
                ...
                <div>
                  ...
                  <h2 className="text-xl md:text-2xl font-bold leading-tight max-w-md">
              ...
              <div className="px-6 pb-6 md:px-8 md:pb-8">
                <p className="text-[10px] text-gray-400 text-center md:text-left">
                  * Tunjukkan tiket ini di loket masuk. <br />* Tiket yang sudah
                  dipindai tidak dapat digunakan kembali.
                </p>
              </div>
            </div>
            ...
            <div className="relative md:w-0 w-full md:h-auto h-0 md:border-l-2 border-t-2 border-dashed border-gray-300 flex-shrink-0">
              <div
                className={`absolute w-8 h-8 rounded-full ${bgPageColor}
                        top-1/2 -left-4 -translate-y-1/2 
                        md:left-1/2 md:top-0 md:-translate-x-1/2 md:-translate-y-1/2
                        shadow-inner box-border z-10`}
              ></div>
              <div
                className={`absolute w-8 h-8 rounded-full ${bgPageColor}
            ...
            {/* === BAGIAN KANAN: STUB QR === */}
            <div className="md:w-80 w-full bg-gray-50 flex flex-col items-center justify-center p-8 relative">
              <h3 className="text-[#0B2F5E] font-bold mb-4 tracking-widest text-sm uppercase">
                Scan Here
              </h3>

              <div className="bg-white p-3 rounded-2xl shadow-sm border border-gray-200 mb-4">
                <QRCodeSVG value={booking.qr_string} size={140} level={"H"} />
              </div>

              {/* Kode Booking dengan Fitur Salin Baru */}
              <div className="text-center w-full">
                <p className="text-xs text-gray-400 mb-1 uppercase">
                  Booking Code
                </p>
                <div
                  className={`flex items-center justify-center gap-2 bg-white border rounded-lg px-3 py-2 cursor-pointer transition group select-none ${
                    isCopied
                ...
                >
                  <span
                    className={`font-mono text-lg font-bold tracking-wider ${
                      isCopied ? "text-green-600" : "text-[#0B2F5E]"
                    }`}
                  >
                  ...
                  {isCopied ? (
                    <Check className="w-4 h-4 text-green-600 transition-all scale-110" />
                  ) : (
                    <Copy className="w-3 h-3 text-gray-400 group-hover:text-[#F57C00] transition-all" />
                  )}
                </div>
                {/* Teks Feedback kecil (Opsional) */}
                <p
                  className={`text-[10px] mt-1 transition-all h-4 ${
                    isCopied ? "text-green-600 opacity-100" : "opacity-0"
                  }`}
                >
                  Berhasil disalin!
                </p>
              ...
              <div className="mt-4">
                <Image
                  ...
                  width={150}
                  height={45}
                  className="opacity-50 transition opacity-100"
                />
              </div>
            </div>
          </div>
