...
// Copy Paste GoogleIcon & FacebookIcon yang sama dari LoginPage di sini
const GoogleIcon = () => ( /* ... code icon google ... */ <svg className="w-5 h-5" viewBox="0 0 24 24"><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4" /><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853" /><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05" /><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335" /></svg> );
...
export default function RegisterPage() {
  // ... state dan handleChange tetap sama ...
  const { login } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    // ... logic submit biasa (tetap sama) ...
    e.preventDefault(); setIsLoading(true); setError('');
    try {
        const res = await fetch('http://localhost:8000/api/register', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(formData) });
    ...
    <div className="min-h-screen flex flex-col bg-[#DCDCDC] font-sans">
       ...
       <header className="bg-white w-full py-4 px-6 md:px-16 flex justify-between items-center shadow-sm">
        <div className="flex items-center gap-4">
            <Link href="/"><Image src="/images/logonama2.png" alt="Logo" width={140} height={40} className="object-contain h-8 md:h-10 w-auto" /></Link>
      ...
      <div className="flex-1 flex items-center justify-center px-4 md:px-16 py-10">
        <div className="w-full max-w-[1200px] grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
            ...
            <div className="hidden lg:flex flex-col justify-center items-center text-center text-white space-y-6">
                <div className="relative w-80 h-80"><Image src="/images/logo-login.png" alt="Ilustrasi" fill className="object-contain drop-shadow-2xl" /></div>
            </div>
            ...
            <div className="w-full max-w-[420px] bg-white rounded-lg shadow-2xl mx-auto lg:ml-auto p-8">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    {/* ... Input Fields (Name, Phone, Email, Password, Confirm) ... */}
