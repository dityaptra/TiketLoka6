...
import { X, CreditCard, Wallet, Banknote, Loader2 } from 'lucide-react';
...
export default function BookingModal({ isOpen, onClose, destination, selectedDate, quantity }: BookingModalProps) {
  ...
  const { token } = useAuth(); // 2. AMBIL TOKEN USER DARI CONTEXT
    ...
    try {
      const res = await fetch('http://127.0.0.1:8000/api/buy-now', {
        ...
        headers: {
          ...
          'Authorization': `Bearer ${token}` // 3. PENTING: KIRIM TOKEN DISINI
        },
        body: JSON.stringify({
      ...
      <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="p-4 bg-gray-50 flex gap-4">
            <div className="h-16 w-16 bg-gray-200 rounded-lg overflow-hidden flex-shrink-0">
               <img src={destination.image_url || ''} alt="" className="w-full h-full object-cover" />
            </div>
            <div>
        ...
        <div className="p-4 border-t bg-white">
          ...
          <button 
            ...
            disabled={loading}
            className="w-full bg-[#0B2F5E] hover:bg-[#061A35] text-white font-bold py-3 rounded-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed flex justify-center items-center gap-2"
          >
