...
import { DestinationImage } from "@/src/types";
...
export default function GalleryManager({
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(
        `http://127.0.0.1:8000/api/admin/destinations/${destinationId}/upload-gallery`,
        {
          method: "POST",
  ...
  const handleDelete = async (imageId: number) => {
    ...
    try {
      const res = await fetch(
        `http://127.0.0.1:8000/api/admin/destinations/gallery/${imageId}`,
        {
          method: "DELETE",
          headers: { Authorization: `Bearer ${token}` },
        }
    ...
    <div className="bg-white p-6 rounded-xl border border-gray-200 mt-6">
      ...
      <div className="mb-8 p-4 bg-gray-50 rounded-lg border border-dashed border-gray-300">
        ...
        <p className="text-xs text-gray-400 mt-2">
          * Bisa pilih banyak foto sekaligus. Max 2MB per foto.
        </p>
      </div>
