...
import { authService } from "@/src/services/authService"; // <--- Pakai Service
...
const FacebookIcon = () => ( <svg className="w-5 h-5" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" fill="#1877F2"/></svg> );
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans relative overflow-hidden bg-white">
      ...
      <div className="flex-1 flex items-center justify-center p-4 md:p-8 pt-20 relative z-10">
        <div className="w-full max-w-5xl bg-white/90 backdrop-blur-sm rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.06)] overflow-hidden grid grid-cols-1 lg:grid-cols-2 min-h-[600px]">
          ...
          <div className="hidden lg:flex flex-col justify-center items-center bg-[#005eff] relative overflow-hidden p-12 text-center">
            ...
            <div className="relative z-10 flex flex-col items-center">
              ...
              <p className="text-blue-100 text-sm max-w-xs leading-relaxed">
                Dapatkan akses ke ribuan event dan destinasi menarik hanya dalam satu aplikasi.
              </p>
            </div>
          </div>
          ...
          <div className="w-full p-8 md:p-12 lg:p-16 flex flex-col justify-center">
            ...
            <form onSubmit={handleSubmit} className="space-y-4 text-gray-600">
              ...
              <div className={inputWrapperClass}>
                ...
                <input name="password" type={showPassword ? "text" : "password"} placeholder="Password (Min. 8 Karakter)" required onChange={handleChange} className={inputClass} />
                <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 text-gray-400 hover:text-gray-600 transition-colors">
              ...
              <div className={inputWrapperClass}>
                <Lock className={iconClass} />
                <input name="password_confirmation" type={showConfirmPassword ? "text" : "password"} placeholder="Konfirmasi Password" required onChange={handleChange} className={inputClass} />
                <button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="absolute right-3 text-gray-400 hover:text-gray-600 transition-colors">
                  {showConfirmPassword ? <EyeOff className="w-5 h-5" /> : <Eye className="w-5 h-5" />}
                </button>
              </div>

              <button type="submit" disabled={isLoading} className="w-full bg-[#F57C00] hover:bg-[#E65100] text-white font-semibold py-3 rounded-lg transition-all transform disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-2 mt-2">
                {isLoading ? <Loader2 className="animate-spin w-5 h-5" /> : "DAFTAR SEKARANG"}
              </button>
            </form>
            ...
            <div className="grid grid-cols-2 gap-4">
              <button type="button" onClick={() => handleSocialLogin('google')} className="flex items-center justify-center gap-2 bg-white border border-gray-200 hover:bg-gray-50 text-gray-700 font-medium py-2.5 rounded-lg transition-all text-sm shadow-sm hover:shadow">
                <GoogleIcon /> <span className="hidden sm:inline">Google</span>
              </button>
              <button type="button" onClick={() => handleSocialLogin('facebook')} className="flex items-center justify-center gap-2 bg-white border border-gray-200 hover:bg-gray-50 text-gray-700 font-medium py-2.5 rounded-lg transition-all text-sm shadow-sm hover:shadow">
            ...
            <div className="mt-6 text-center text-xs text-gray-600 leading-relaxed px-4 border-t pt-4 border-gray-100">
              Dengan mendaftar, Anda menyetujui{" "}
              <Link href="/terms" className="underline text-[#F57C00]">Kebijakan Privasi</Link> dan{" "}
              <Link href="/privacy" className="underline text-[#F57C00]">Syarat & Ketentuan</Link> TiketLoka.
            </div>
          </div>
        </div>
      </div>
