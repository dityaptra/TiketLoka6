...
import {
  Calendar, ChevronRight, Ticket, Loader2, AlertCircle, MapPin, Hash, Search, Clock, CheckCircle2, XCircle, CreditCard
} from "lucide-react";

import { useAuth } from "@/src/context/AuthContext";
import { bookingService } from "@/src/services/bookingService";
import { Booking } from "@/src/types";
import { formatIDR, getImageUrl, formatDate } from "@/src/lib/utlis";
...
export default function MyTicketsPage() {
  ...
  // Ratakan data tiket dari booking details
  const allTickets = useMemo(() => {
    return bookings.flatMap((booking) =>
      booking.details.map((detail, index) => ({
        ...detail,
        parent_status: booking.status, // Ambil status dari parent
  ...
  const filteredTickets = allTickets.filter((ticket) => {
    const matchesStatus = activeTab === "all" || ticket.parent_status === activeTab;
    const matchesSearch =
        ...
        <div className="max-w-6xl mx-auto relative z-10">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
            ...
            <div className="relative w-full md:w-80">
              ...
              <input
                type="text"
                className="block w-full pl-10 pr-3 py-3 border-none rounded-xl bg-white/10 text-white placeholder-blue-200 focus:outline-none focus:bg-white/20 focus:ring-2 focus:ring-[#F57C00] transition-all sm:text-sm shadow-inner"
      ...
      <div className="max-w-6xl mx-auto px-4 -mt-8 pb-20 relative z-20">
        {loading || authLoading ? (
          ...
          <div className="text-center py-16 text-gray-500 bg-white rounded-3xl shadow-sm p-8 border border-gray-100 max-w-2xl mx-auto">
            ...
            <p className="font-medium text-gray-500 mb-6">Silakan login terlebih dahulu untuk melihat riwayat tiket Anda.</p>
            <Link href="/login" className="inline-flex items-center gap-2 bg-[#0B2F5E] text-white px-8 py-3 rounded-xl font-bold hover:bg-[#09254A] transition shadow-lg hover:shadow-xl transform hover:-translate-y-1">
          ...
          <div className="text-center py-20 bg-white rounded-3xl shadow-sm p-8 border border-gray-100">
            <div className="w-20 h-20 bg-gray-50 rounded-full flex items-center justify-center mx-auto mb-4 border border-gray-100">
              {searchQuery ? <Search className="w-8 h-8 text-gray-400" /> : <Ticket className="w-8 h-8 text-gray-400" />}
            </div>
            <h3 className="text-lg font-bold text-gray-800">{searchQuery ? "Tiket tidak ditemukan" : "Belum ada tiket"}</h3>
            <p className="text-gray-500 mt-1 max-w-md mx-auto">
              {searchQuery ? `Tidak ada hasil untuk pencarian "${searchQuery}"` : "Anda belum memesan tiket wisata apapun. Yuk mulai petualanganmu!"}
            </p>
            {!searchQuery && (
              <Link href="/" className="mt-6 inline-block bg-[#F57C00] text-white px-8 py-3 rounded-xl font-bold shadow-lg shadow-orange-200 hover:shadow-orange-300 transition hover:-translate-y-0.5">
                Cari Wisata
              </Link>
            )}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredTickets.map((ticket, idx) => {
              // --- LOGIKA WARNA & STATUS ---
              // Cast ke string untuk menghindari error TypeScript jika tipe data belum diupdate
              const status = ticket.parent_status as string; 
              ...
              if (status === "success" || status === "paid" || status === "settlement") {
                ...
                StatusIcon = Clock;
              } else if (status === "cancelled" || status === "failed" || status === "deny") {
              ...
              // Tentukan URL tujuan saat kartu diklik
              ...
              // --- PERBAIKAN NAN & HARGA ---
              // 1. Pastikan harga dikonversi ke Number()
              const pricePerItem = Number(ticket.destination?.price) || 0;
              const quantity = Number(ticket.quantity) || 0;
              const subtotal = Number(ticket.subtotal);

              // 2. Hitung total (Prioritas: Subtotal dari DB -> Hitung Manual -> 0)
              const finalPrice = subtotal || (pricePerItem * quantity) || 0;

              return (
                <Link
                  href={targetUrl}
                  key={`${ticket.parent_id}-${ticket.id}-${idx}`}
                  className={`group bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-xl transition-all duration-300 flex flex-col ${isPending ? "hover:border-orange-300" : "hover:border-blue-100"}`}
                ...
                >
                  <div className="relative h-48 w-full overflow-hidden">
                    ...
                    <div className="absolute top-3 right-3">
                      <span className={`text-xs font-bold px-3 py-1.5 rounded-full uppercase tracking-wide border flex items-center gap-1.5 backdrop-blur-md shadow-sm ${statusColor}`}>
                        <StatusIcon className="w-3.5 h-3.5" /> {status === "paid" ? "Lunas" : status}
                      </span>
                    </div>
                  </div>
                  <div className="p-5 flex-1 flex flex-col">
                    ...
                    <div className="space-y-2 mb-4 flex-1">
                      <div className="flex items-center text-sm text-gray-600 gap-2.5">
                        ...
                        <span className="font-medium">{formatDate(ticket.visit_date)}</span>
                      </div>
                      <div className="flex items-center text-sm text-gray-600 gap-2.5">
                    ...
                    <div className="pt-4 border-t border-gray-100 flex items-center justify-between mt-auto">
                      <div className="flex flex-col">
                        <span className="text-xs text-gray-400">{isPending ? "Total Tagihan" : "Total Harga"}</span>
                        {/* ✅ TAMPILKAN HARGA YANG SUDAH DIPERBAIKI (No NaN) */}
                        <span className="font-bold text-[#0B2F5E] text-sm">{formatIDR(finalPrice)}</span>
                      </div>
                      {isPending ? (
