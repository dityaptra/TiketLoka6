...
import { Plus, Trash2, Edit, Search, ImageIcon, Loader2, Filter } from "lucide-react";
import Link from "next/link";
import Swal from "sweetalert2";

// Import Service, Types, Utils
import { adminService } from "@/src/services/adminService";
import { Destination, Category } from "@/src/types";
...
export default function AdminDestinations() {
  ...
  async function fetchData() {
    setLoading(true);
    try {
      // Panggil Service (Parallel Fetch)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white border-b border-gray-200 sticky top-0 z-20 px-6 py-4 shadow-sm flex justify-between items-center rounded-lg">
        ...
        <Link href="/admin/destinations/create" className="bg-[#0B2F5E] text-white px-4 py-2 rounded-lg font-bold text-sm flex items-center gap-2 hover:bg-[#061A35] transition shadow-md">
      ...
      <div className="bg-white p-2 rounded-xl shadow-sm border border-gray-200 flex flex-col md:flex-row items-center gap-2">
        ...
        <button onClick={handleSearch} className="bg-[#0B2F5E] text-white px-6 py-2 rounded-lg text-sm font-bold hover:bg-[#09254A] transition w-full md:w-auto shadow-sm">
          Cari
        </button>
            ...
            {filteredData.length > 0 ? (
              filteredData.map((item, index) => (
                <tr key={item.id} className="hover:bg-blue-50/50 transition-colors">
                  <td className="px-6 py-4 align-middle text-center font-bold text-gray-600">{index + 1}</td>
                  ...
                  <td className="px-6 py-4 align-middle"><h3 className="font-bold text-[#0B2F5E] text-base">{item.name}</h3></td>
                  <td className="px-6 py-4 align-middle">
                    <span className="text-blue-700 px-3 py-1 rounded-full text-xs font-bold">{item.category?.name || "Umum"}</span>
                  </td>
                  <td className="px-6 py-4 align-middle"><div className="flex items-center gap-1.5 text-gray-600 text-sm">{item.location}</div></td>
                  <td className="px-6 py-4 align-middle">
                    <span className="font-bold text-gray-700 text-sm px-3 py-1 rounded-md">Rp {Number(item.price).toLocaleString("id-ID")}</span>
                  </td>
                  <td className="px-6 py-4 align-middle text-center">
                    <div className="flex justify-center gap-2">
                      <Link href={`/admin/destinations/edit/${item.id}`} className="p-2 bg-white border border-gray-200 text-blue-600 rounded-lg hover:bg-blue-50 hover:border-blue-200 transition shadow-sm" title="Edit">
                        <Edit size={16} />
                      </Link>
                      <button
                        onClick={() => handleDelete(item.id)}
                        disabled={deletingId === item.id}
                        className={`p-2 border rounded-lg transition shadow-sm ${deletingId === item.id ? "bg-red-50 border-red-200 cursor-not-allowed" : "bg-white border-gray-200 text-red-600 hover:bg-red-50 hover:border-red-200"}`}
                        title="Hapus"
                      >
                        {deletingId === item.id ? <Loader2 size={16} className="animate-spin text-red-600" /> : <Trash2 size={16} />}
                      </button>
                    </div>
                  </td>
                </tr>
