...
interface BookingDetail {
  ...
  payment_method: string; // <--- TAMBAHAN PENTING
  created_at: string;
  destination: {
    name: string;
    location: string;
...
export default function PaymentPage() {
  ...
  const fetchBooking = useCallback(async () => {
    try {
      const res = await fetch(
        `http://127.0.0.1:8000/api/bookings/${params.code}`,
        {
  ...
  const handleCheckStatus = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `http://127.0.0.1:8000/api/bookings/${booking?.booking_code}/pay`,
        {
          method: "POST",
          headers: {
  ...
  const handleCancelOrder = async () => {
    ...
    try {
      const res = await fetch(
        `http://127.0.0.1:8000/api/bookings/${booking?.booking_code}/cancel`,
        {
          method: "POST",
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FA] font-sans pb-20">
      ...
      <div className="max-w-xl mx-auto px-4 pt-28">
        <button
          onClick={() => router.back()}
          className="flex items-center gap-2 text-gray-500 hover:text-[#0B2F5E] mb-6 transition font-medium text-sm"
        >
          ...
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="bg-orange-50 border border-orange-100 rounded-2xl p-5 flex items-center justify-between mb-6 shadow-sm">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-[#F57C00] font-extrabold text-2xl font-mono">
                    {formatTime(timeLeft)}
                  </p>
                </div>
              </div>
              <span className="text-[10px] bg-orange-200 text-orange-800 px-2.5 py-1 rounded-full font-bold uppercase tracking-wider">
                Pending
              </span>
            </div>
            ...
            <div className="bg-white rounded-3xl shadow-xl shadow-gray-200/50 border border-gray-100 overflow-hidden relative">
              ...
              <div className="bg-[#0B2F5E] p-8 text-center text-white relative overflow-hidden">
                ...
                <p className="text-sm text-blue-100 mb-1 font-medium relative z-10">
                  Total Tagihan
                </p>
                <div className="flex items-center justify-center gap-2 relative z-10">
                  ...
                  <button
                    onClick={() => copyToClipboard(String(booking.grand_total))}
                ...
                <div className="mt-4 flex justify-center relative z-10">
                  <span className="text-xs bg-white/20 px-4 py-1.5 rounded-full font-medium text-white backdrop-blur-sm border border-white/10">
              ...
              <div className="p-8 flex flex-col items-center">
                {isQRIS ? (
                  ...
                  <>
                    <div className="text-center mb-8">
                      <h3 className="font-bold text-gray-900 text-xl flex items-center gap-2 justify-center">
                        <ScanLine className="w-5 h-5 text-[#0B2F5E]" /> Scan
                        QRIS
                      </h3>
                      <p className="text-gray-500 text-sm mt-1">
                        Gunakan GoPay, OVO, Dana, BCA, dll.
                      </p>
                    </div>
                    <div className="relative group bg-white p-4 rounded-3xl border-2 border-dashed border-gray-300 mb-8 transition-all hover:border-[#0B2F5E]">
                      <QRCodeSVG
                        value={
                          booking.qris_string ||
                          `TIKETLOKA-${booking.booking_code}`
                        }
                        size={220}
                        level="H"
                        includeMargin={true}
                      ...
                      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                        <div className="bg-white px-2 py-1 rounded font-bold text-[10px] text-[#0B2F5E] shadow-sm border border-gray-100">
                          QRIS
                        </div>
                      </div>
                    </div>
                  ...
                  <>
                    <div className="text-center mb-8">
                      <h3 className="font-bold text-gray-900 text-xl flex items-center gap-2 justify-center">
                        <Building2 className="w-5 h-5 text-[#0B2F5E]" />{" "}
                        Transfer Bank
                      </h3>
                      <p className="text-gray-500 text-sm mt-1">
                    ...
                    <div className="w-full bg-blue-50 border border-blue-100 p-6 rounded-2xl mb-8 relative overflow-hidden">
                      ...
                      <div className="flex items-center gap-3 mb-4">
                        <p className="font-mono text-2xl font-bold text-[#0B2F5E] tracking-wider">
                          {bankAccount}
                        </p>
                        <button
                          onClick={() =>
                            copyToClipboard(bankAccount.replace(/\s/g, ""))
                          }
                          className="p-2 bg-white rounded-lg shadow-sm text-blue-600 hover:bg-blue-100"
                        >
                          <Copy size={16} />
                        </button>
                      </div>
            ...
            <div className="mt-6 bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
              <h4 className="font-bold text-gray-800 mb-4 flex items-center gap-2 text-sm uppercase tracking-wider">
                <AlertCircle size={18} className="text-blue-500" /> Cara
                Pembayaran
              </h4>
                ...
                {isQRIS
                  ? [
                      ...
                      <li key={i} className="flex gap-3 text-sm text-gray-600">
                        <span className="flex-shrink-0 w-5 h-5 rounded-full bg-blue-50 text-blue-600 flex items-center justify-center font-bold text-xs">
                          {i + 1}
                        </span>
                        {step}
                      </li>
        ...
        {booking.status === "cancelled" && (
          <div className="flex flex-col items-center justify-center py-24 text-center bg-white rounded-3xl shadow-sm border border-gray-100 animate-in zoom-in-95">
            ...
            <button
              onClick={() => router.push("/")}
              className="bg-[#0B2F5E] text-white px-8 py-3.5 rounded-xl font-bold hover:bg-[#09254A] transition shadow-lg"
            >
              Cari Wisata Lain
            </button>
          </div>
